...
import bhargav.server.LeaderboardServerGUI;
...
public class QuizGUI extends Application {
    ...
    public void start(Stage stage) {
        ...
        hostButton.setOnAction(event -> handleOpenLeaderboard(event));
        hostButton.setStyle(
                "-fx-font-size: 20px;" +
                        "-fx-font-family: 'Lucida Console';" +
                        "-fx-text-fill: white;" +
        ...
        hostButton.setOnMouseEntered(_ -> hostButton.setStyle(
                "-fx-font-size: 20px;" +
                        "-fx-font-family: 'Lucida Console';" +
                        "-fx-text-fill: #add8e6;" + // Soft blue
                        "-fx-background-color: royalblue;" + // Richer blue
        ...
        hostButton.setOnMouseExited(_ -> hostButton.setStyle(
                "-fx-font-size: 20px;" +
                        "-fx-font-family: 'Lucida Console';" +
                        "-fx-text-fill: white;" +
                        "-fx-background-color: darkblue;" + // Different color
        ...
        buttonBox.getChildren().addAll(startButton, playOnlineButton, hostButton);
        ...
        primaryStage.setOnCloseRequest(_ -> {
            if (leaderboardClient != null) {
                leaderboardClient.disconnectFromServer();
            }
        });
            ...
            try {
                ...
                javafx.concurrent.Task<String> connectionTask = new javafx.concurrent.Task<String>() {
                    ...
                    protected String call() throws Exception {
                        ...
                        return leaderboardClient.connectAndNotify(username, serverIp, serverPort);
                    }
                };
    ...
    private void showfirstQuestion() {
        ...
        // Optional: Add a small speaker icon above the slider
        ImageView speakerIcon = new ImageView();
        try {
            speakerIcon.setImage(new Image(getClass().getResourceAsStream("/images/speaker.png")));
            speakerIcon.setFitWidth(22);
            speakerIcon.setFitHeight(22);
        } catch (Exception e) {
            // fallback: no icon
        }
        ...
        volumeSlider.setStyle(
                "-fx-background-color: #1a0033;" +
                        ...
                        "-fx-border-radius: 10px;" +
                        "-fx-background-radius: 10px;" +
                        "-fx-effect: dropshadow(three-pass-box, gold, 4, 0.3, 0, 0);");
        ...
        StackPane.setAlignment(volumeSlider, Pos.BOTTOM_LEFT);
        StackPane.setMargin(volumeSlider, new Insets(0, 0, 40, 32));
        ...
        rootStack.getChildren().addAll(quizBackground, quizWrapper, pauseButton, volumeSlider);
    ...
    private void showPauseMenu() {
        ...
        Button quitButton = createStyledPauseButton("Quit", () -> {
            primaryStage.close();
            leaderboardClient.disconnectFromServer();
        });
        ...
        if (onlineMode) {
            ...
            javafx.concurrent.Task<String> sendScoreTask = new javafx.concurrent.Task<String>() {
                ...
                protected String call() throws Exception {
                    ...
                    return leaderboardClient.sendScoreToServer(username, calculatedScore, timeTaken, correctAnswers,
                            serverIp, serverPort);
                }
            };
    ...
    @FXML
    private void handleOpenLeaderboard(ActionEvent event) {
        LeaderboardServerGUI leaderboardGUI = new LeaderboardServerGUI();
        Parent root = leaderboardGUI.getRoot();

        Scene scene = new Scene(root);
        Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
        stage.setScene(scene);
        stage.setTitle("Leaderboard Server");
